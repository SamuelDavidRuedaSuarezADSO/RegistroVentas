    ...
    .then((r)=>{
        r.forEach(e => {
            ...
            dele.addEventListener("click", (event)=>{
                ...
                let confirmar = confirm("¿Esta seguro de eliminar este COLOR?")
                if(confirmar){
                    eliminar(cod.textContent, `colores`);
                    limpiar();
    ...
    if(resp){
        ...
        listar(`colores`)
            .then((x)=>{
                ...
                if(existe){
                    alert("ERROR: El CODIGO del COLOR ya esta en uso");
                }
                else{
                    const data = {
                        id: $cod.value,
                    ...
                    registrar(data, `colores`);
                    limpiar();
                    alert("El COLOR fue registrado con exito");
                }
            })     
    }
});
...
$modificar.addEventListener("click", ()=>{
    if($cod.value != "" || $nom.value != ""){
        ...
        modificar($cod.value, datos, `colores`);
        alert("El COLOR fue modificada con exito");
        limpiar();
    }
    else{
...
$delete.addEventListener("click", ()=>{
    if($cod.value != "" || $nom.value != ""){
        let confirmar = confirm("¿Esta reguro de eliminar este COLOR?")
        if(confirmar){
            eliminar($cod.value, `colores`);
            alert("El COLOR fue eliminada con exito");
            limpiar();
        }
    }
    else{
...
$buscarForm.addEventListener("submit", (event)=>{
    ...
    if($buscarInput.value != ""){
        buscar($buscarInput.value, `colores`)
            .then((c)=>{
                $cod.value = c.id;
                $nom.value = c.nombre;
            })
            .catch(()=>{
                alert("ERROR: COLOR no encontrada");
            })
    }
    else{
